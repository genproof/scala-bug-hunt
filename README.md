...
- ![status](https://img.shields.io/github/issues/detail/state/com-lihaoyi/scalasql/113) [#113](https://github.com/com-lihaoyi/scalasql/issues/113) — take(m).drop(n) with n > m generates negative LIMIT, rejected by every supported dialect
- ![status](https://img.shields.io/github/issues/detail/state/com-lihaoyi/scalasql/114) [#114](https://github.com/com-lihaoyi/scalasql/issues/114) — onConflictUpdate with no update assignments emits invalid SQL ("DO UPDATE SET")
- ![status](https://img.shields.io/github/issues/detail/state/com-lihaoyi/scalasql/115) [#115](https://github.com/com-lihaoyi/scalasql/issues/115) — OptionType.put always passes JDBCType.NULL to setNull, ignoring the column's actual JDBC type
- ![status](https://img.shields.io/github/issues/detail/state/com-lihaoyi/scalasql/116) [#116](https://github.com/com-lihaoyi/scalasql/issues/116) — Config.camelToSnake merges consecutive uppercase letters (XMLParser -> xmlparser, not xml_parser)
- ![status](https://img.shields.io/github/issues/detail/state/com-lihaoyi/scalasql/117) [#117](https://github.com/com-lihaoyi/scalasql/issues/117) — WithCte.filter swaps lhs/rhs — WHERE ends up inside the CTE body, not on the outer query
...
- ![status](https://img.shields.io/github/issues/detail/state/dotty-cps-async/dotty-cps-async/119) [#119](https://github.com/dotty-cps-async/dotty-cps-async/issues/119) — EitherAsyncShift.forall returns false for Left (should be true)
- ![status](https://img.shields.io/github/issues/detail/state/dotty-cps-async/dotty-cps-async/120) [#120](https://github.com/dotty-cps-async/dotty-cps-async/issues/120) — TryAsyncShift.recover/recoverWith throw MatchError on unhandled exceptions
- ![status](https://img.shields.io/github/issues/detail/state/dotty-cps-async/dotty-cps-async/121) [#121](https://github.com/dotty-cps-async/dotty-cps-async/issues/121) — UsingAsyncShift.apply leaks the resource on the failure path
- ![status](https://img.shields.io/github/issues/detail/state/dotty-cps-async/dotty-cps-async/122) [#122](https://github.com/dotty-cps-async/dotty-cps-async/issues/122) — IndexedSeqAsyncShift.indexWhere ignores the from parameter
- ![status](https://img.shields.io/github/issues/detail/state/dotty-cps-async/dotty-cps-async/123) [#123](https://github.com/dotty-cps-async/dotty-cps-async/issues/123) — ArrayOpsAsyncShift.dropWhile returns Array(head) instead of empty when all elements match
- ![status](https://img.shields.io/github/issues/detail/state/dotty-cps-async/dotty-cps-async/124) [#124](https://github.com/dotty-cps-async/dotty-cps-async/issues/124) — BreaksAsyncShift.breakable discards the body computation via value-discarding to Unit
- ![status](https://img.shields.io/github/issues/detail/state/dotty-cps-async/dotty-cps-async/125) [#125](https://github.com/dotty-cps-async/dotty-cps-async/issues/125) — ArrayOpsAsyncShift.lastIndexWhere doesn't clamp end, throws IndexOutOfBoundsException
...
- ![status](https://img.shields.io/github/issues/detail/state/flink-extended/flink-scala-api/372) [#372](https://github.com/flink-extended/flink-scala-api/issues/372) — EitherTypeInfo.equals / hashCode NPE when leftTypeInfo or rightTypeInfo is null
- ![status](https://img.shields.io/github/issues/detail/state/flink-extended/flink-scala-api/373) [#373](https://github.com/flink-extended/flink-scala-api/issues/373) — MappedTypeInformation.hashCode ignores mapper, equals checks both — contract violation and systematic collisions
- ![status](https://img.shields.io/github/issues/detail/state/flink-extended/flink-scala-api/374) [#374](https://github.com/flink-extended/flink-scala-api/issues/374) — OptionTypeInfo.createComparator throws with truncated message "Element type that doesn't support "
- ![status](https://img.shields.io/github/issues/detail/state/flink-extended/flink-scala-api/375) [#375](https://github.com/flink-extended/flink-scala-api/issues/375) — CoproductSerializer subtype-index byte overflow for sealed traits with >127 subtypes
...
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1052) [#1052](https://github.com/propensive/soundness/issues/1052) — parasite Promise.await(duration) passes relative nanos to LockSupport.parkUntil (expects absolute millis)
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1053) [#1053](https://github.com/propensive/soundness/issues/1053) — feudalism Semaphore.access / Semaphore.isolate use notify() instead of notifyAll() — lost wakeups
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1054) [#1054](https://github.com/propensive/soundness/issues/1054) — hallucination Raster.crop swaps top/bottom in the pixel mapping
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1055) [#1055](https://github.com/propensive/soundness/issues/1055) — profanity Signal.id formula returns wrong POSIX numbers (SIGHUP becomes -1)
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1056) [#1056](https://github.com/propensive/soundness/issues/1056) — telekinesis Context backs session storage with an unsynchronised mutable HashMap
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1057) [#1057](https://github.com/propensive/soundness/issues/1057) — gastronomy Long Digestible uses wrong shift range and loses two bytes per Long
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1058) [#1058](https://github.com/propensive/soundness/issues/1058) — denominative Ordinal.subsequent(size) returns an Interval of size+1 (off-by-one)
- ![status](https://img.shields.io/github/issues/detail/state/propensive/soundness/1059) [#1059](https://github.com/propensive/soundness/issues/1059) — revolution manifestAttributes objects misspell ``SpecificationVendor`` / ``SpecificationVersion`` as ``Specifacation*
...
- ![status](https://img.shields.io/github/issues/detail/state/sirthias/borer/829) [#829](https://github.com/sirthias/borer/issues/829) — Reader.longCompare returns the wrong sign due to Long subtraction overflow

### [softwaremill/magnolia](https://github.com/softwaremill/magnolia)

- ![status](https://img.shields.io/github/issues/detail/state/softwaremill/magnolia/646) [#646](https://github.com/softwaremill/magnolia/issues/646) — SealedTrait.Subtype.typeAnnotations contains parent's paramTypeAnns instead of subtype's typeAnns
- ![status](https://img.shields.io/github/issues/detail/state/softwaremill/magnolia/647) [#647](https://github.com/softwaremill/magnolia/issues/647) — Monadic[Future].point uses Future(value) instead of Future.successful(value)
...
- ![status](https://img.shields.io/github/issues/detail/state/tarao/record4s/169) [#169](https://github.com/tarao/record4s/issues/169) — record4s-circe encoder produces non-deterministic JSON field order
- ![status](https://img.shields.io/github/issues/detail/state/tarao/record4s/170) [#170](https://github.com/tarao/record4s/issues/170) — Selector.of[T] creates a Selector with empty labels — unapply returns EmptyTuple cast to ElemTypes
